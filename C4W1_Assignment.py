...
class CrossAttention(tf.keras.layers.Layer):
    ...
    def call(self, context, target):
        ...
        attn_output = self.mha(key=context,
            query=target,
            value=context
        )  
...
def translate(model, text, max_length=50, temperature=0.0):
    ...
    # Vectorize the text using the correct vectorizer (we assume english_vectorizer is available)
    context = english_vectorizer(texts_tensor).to_tensor()  # Vectorizer for the English input
        
    # Get the encoded context (pass the context through the encoder)
    # Hint: Remember you can get the encoder by using model.encoder
    encoded_context = model.encoder(context)
    
    # INITIAL STATE OF THE DECODER
    
    # First token should be SOS token with shape (1,1)
    next_token = tf.fill((1, 1),sos_id)  # Start-of-sequence token
    
    # Initial hidden and cell states should be tensors of zeros with shape (1, UNITS)
    state = [tf.zeros((1, UNITS)), tf.zeros((1, UNITS))]
    
    # You are done when you draw an EOS token as the next token
    done = False
    ...
    for _ in range(max_length):
        ...
        try:
            # Generate the next token from the decoder (state is updated as well)
            next_token, logit, state, done = model.decoder(
            ...
            raise Exception(f"Error generating the next token: {e}")
        
        # If done, break out of the loop (EOS token)
        if done:
            break
        
        # Add the predicted token to the list of tokens
        tokens.append(next_token)
        
        # Add the logit to the list of logits (to capture certainty of prediction)
        logits.append(logit)
    ...
    # Convert the translated tokens back to text using the tokenizer
    translated_text = model.decoder.tokenizer.sequences_to_texts(tokens.numpy())[0]

    return translated_text, logits[-1], tokens
