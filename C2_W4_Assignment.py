...
def back_prop(x, yhat, y, h, W1, W2, b1, b2, batch_size):
    ...
    delta2 = yhat - y
    # Compute the gradient for W2 (dL/dW2)
    grad_W2 = (1 / batch_size) * np.dot(delta2, h.T)
    
    # Compute the gradient for b2 (dL/db2)
    grad_b2 = (1 / batch_size) * np.sum(delta2, axis=1, keepdims=True)
    
    # Backpropagate the error to the hidden layer (delta1)
    delta1 = np.dot(W2.T, delta2)  # Backpropagate through W2
    
    # Apply ReLU derivative (if ReLU was used in the hidden layer)
    delta1[h <= 0] = 0  # Zero out the gradient where ReLU was inactive
    
    # Compute the gradient for W1 (dL/dW1)
    grad_W1 = (1 / batch_size) * np.dot(delta1, x.T)
    
    # Compute the gradient for b1 (dL/db1)
