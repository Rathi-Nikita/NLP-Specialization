...
def line_to_tensor(line, vocab):
    ...
    char_lookup = tf.keras.layers.StringLookup(vocabulary=vocab, output_mode='int', mask_token=None)
...
def log_perplexity(preds, target):
    ...
    target_one_hot = np.eye(preds.shape[-1])[target]
    log_p = np.sum(target_one_hot * preds.numpy(), axis= -1) # HINT: tf.one_hot(...) should replace one of the Nones
    # Identify non-padding elements in the target
    non_pad = 1.0 - np.equal(target, PADDING_ID).astype(np.float32)        # You should check if the target equals to PADDING_ID
