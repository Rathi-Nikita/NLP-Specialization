...
def viterbi_backward(best_probs, best_paths, corpus, states):
    ...
    # Iterate over all possible POS tags for the last word (last column in best_probs)
    for k in range(best_probs.shape[0]):  # number of tags
        if best_probs[k, m - 1] > best_prob_for_last_word:
            best_prob_for_last_word = best_probs[k, m - 1]
            z[m - 1] = k  # Store the index of the best POS tag for the last word
    ...
    for i in range(m - 2, -1, -1): # complete this line
