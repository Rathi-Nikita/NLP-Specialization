...
def get_sentence_vectorizer(sentences):
    ...
    sentence_vectorizer = tf.keras.layers.TextVectorization(
        output_mode='int',   # Converts text to integer indices
        output_sequence_length=None,
        standardize=None # No fixed length for sequences, since it should adapt to varying sentence lengths
    )
...
def label_vectorizer(labels, tag_map):
    ...
    for element in labels:
        ...
        for token in tokens:
            element_ids.append(tag_map.get(token, -1))
...
def NER(len_tags, vocab_size, embedding_dim = 50):
    ...
    model.add(tf.keras.layers.Embedding(input_dim=vocab_size + 1,  # Adding 1 for padding (if mask_zero=True)
...
def masked_loss(y_true, y_pred):
    ...
    loss_fn = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True, reduction=tf.keras.losses.Reduction.NONE, ignore_class=-1)
...
def masked_accuracy(y_true, y_pred):
    ...
    matches_true_pred = tf.equal(y_true, tf.cast(y_pred_class, tf.float32))
...
def predict(sentence, model, sentence_vectorizer, tag_map):
    ...
    for tag_idx in outputs:
        pred_label = tag_map.get(tag_idx, "O")     
