...
def get_corrections(word, probs, vocab, n=5, verbose = False):
    ...
    suggestions = []
    # If the word is in the vocabulary, suggest it immediately
    if word in vocab:
        suggestions = [word]
    
    # Otherwise, try generating suggestions with one edit
    suggestions = suggestions or list(edit_one_letter(word).intersection(vocab))
    
    # If still no suggestions, try generating suggestions with two edits
    suggestions = suggestions or list(edit_two_letters(word).intersection(vocab))
    
    # Create a list of tuples (word, probability)
    best_words = [(s, probs.get(s, 0)) for s in suggestions]
    
    # Sort suggestions by probability (in descending order) and take the top n
    n_best = sorted(best_words, key=lambda x: x[1], reverse=True)[:n]
    
    if verbose:
        print("Suggestions = ", suggestions)
        print(f"Best {n} suggestions: {n_best}")
    return n_best
